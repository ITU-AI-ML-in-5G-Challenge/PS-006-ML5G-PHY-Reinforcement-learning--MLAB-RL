...
import os
import caviar_tools
from beamselect_env_customState_test import BeamSelectionEnv
...
'''
Tests an PPO2 network previously trained and stored in file.
'''
def test_beam_selection(model_path, model_name, test_ep, full_output):
    ...
    for ep in test_ep:
        ...
        obs = env.reset()
        if full_output == True:
            output_file = './result/ppo2/'+'output_'+model_name+'_ep'+str(ep)+'.csv'
            ...
            output_file = './result/ppo2/'+'actions_'+model_name+'_ep'+str(ep)+'.csv'
            with open(output_file, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
...
$ python3 test_b-ppo2.py -m <model_name> -ep <test_ep_id#first> <test_ep_id#last> --full-output <if not then only actions>

Example:

$ python3 test_b-ppo2.py -m baseline.ppo2 -ep 0 1 --full-output
'''
